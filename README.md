...
Vendor Matching
The Problem: Hauler vendor names arrive in inconsistent formats from Document AI parsing. For example, "Republic Services" may appear as:

REPUBLIC SERVICES
REPUBLIC\nSERVICES
Republic Services Inc.
REPUBLIC
The Solution: A two-stage fuzzy matching algorithm normalizes messy vendor names to our canonical vendor list from vw_flat_services:

Direct Match: Invoice vendor name matched against clean vendor list using multiple techniques (exact match, fuzzy match, substring match)
Location Match: For remaining invoices, use counterparty → location → vendor lookup
...
Fresh data exported from SQL Server
Pipeline script processes and generates output files
Files pushed to GitHub for dashboard refresh
Note: Dashboard shows data through the previous day (current day excluded).

Limitations
Unmatched Invoices: ~1.2% of invoices cannot be matched to a known vendor
Document AI Accuracy: Source data quality depends on OCR parsing
Manual Updates: Dashboard requires daily manual refresh
Historical Only: This is not a real-time feed
Business Value
